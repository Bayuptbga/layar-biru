...
function openAddFilmModal() {
  ...
  // Preview thumbnail otomatis dari Google Drive File ID
  const videoIdEl = document.getElementById('af-videoid');
  if (videoIdEl) {
    videoIdEl.oninput = function () {
...
async function submitAddFilm() {
  ...
  const videoId  = document.getElementById('af-videoid').value.trim(); // Google Drive file ID
  ...
  if (!title || !desc || !videoId) {
    errEl.textContent = 'Judul, deskripsi, dan Google Drive File ID wajib diisi.';
  ...
  try {
    // Kirim sebagai gdriveId agar server buat embed & thumb otomatis
    const res  = await fetch(`${API_BASE}/api/films`, {
      method:  'POST',
      headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${authToken}` },
      body:    JSON.stringify({ title, desc, gdriveId: videoId })
    });
    const data = await res.json();
    ...
    renderFilmGrid();
    closeAddFilmModal();
    addAdminLog('Admin', `Film ditambahkan: ${title} (Google Drive)`, '#5B8CFF', 'info');
...
// Sync semua video dari Google Drive folder
async function syncFromGDrive() {
  ...
  try {
    const res  = await fetch(`${API_BASE}/api/gdrive/sync`, {
      method:  'POST',
      headers: { 'Authorization': `Bearer ${authToken}` }
    });
    const data = await res.json();
    if (data.success) {
      ...
      data.films.forEach(f => FILMS.push(f));
      renderFilmGrid();
      addAdminLog('Admin', `Sync GDrive selesai: ${data.count} video ditemukan`, '#4ADE80', 'info');
      alert(`✅ Berhasil sync ${data.count} video dari Google Drive!`);
    } else {
      alert('Gagal sync: ' + (data.message || 'Unknown error'));
    }
  } catch (err) {
    alert('Error: ' + err.message);
  } finally {
    if (btn) { btn.disabled = false; btn.textContent = '🔄 Sync GDrive'; }
  }
}
